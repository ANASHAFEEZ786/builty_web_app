...
import { Fuel, Search, ChevronLeft, ChevronRight, Save, X, Trash2, LogOut, Plus, Edit } from 'lucide-react';
...
const DieselStationMaster = () => {
    ...
    const [records, setRecords] = useState([
        { id: 1, code: '01', name: 'PSO LAHORE MAIN', location: 'Lahore', contactNo: '0300-1234567' },
        { id: 2, code: '02', name: 'SHELL KARACHI PORT', location: 'Karachi', contactNo: '0321-7654321' },
        { id: 3, code: '03', name: 'TOTAL PARCO M2', location: 'Motorway M2', contactNo: '0333-1111111' },
        { id: 4, code: '04', name: 'BYCO MULTAN', location: 'Multan', contactNo: '0345-2222222' },
    ]);

    const [formData, setFormData] = useState(records[0]);

    const handleInputChange = (field, value) => setFormData({ ...formData, [field]: value });
    const handleAddNew = () => { setMode('add'); setFormData({ id: records.length + 1, code: '', name: '', location: '', contactNo: '' }); };
    const handleEdit = () => setMode('edit');
    const handleSave = () => {
        if (mode === 'add') { setRecords([...records, formData]); setCurrentIndex(records.length); }
        else { setRecords(records.map((r, i) => i === currentIndex ? formData : r)); }
        setMode('view');
    };
    ...
    const handleDelete = () => {
        if (records.length > 1) {
            ...
            setRecords(filtered); setCurrentIndex(Math.max(0, currentIndex - 1)); setFormData(filtered[Math.max(0, currentIndex - 1)]);
        }
    };
    ...
    return (
        <div className={`space-y-6 text-text-main transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between">
                <div><h2 className="text-3xl font-bold gradient-text tracking-tight">Diesel Station Master</h2><p className="text-slate-400 mt-1">Manage diesel/fuel stations</p></div>
                <span className="px-3 py-1.5 bg-slate-800/50 rounded-lg text-xs text-slate-400">Record {currentIndex + 1} of {records.length}</span>
            </div>
