...
import { Fuel, Search, ChevronLeft, ChevronRight, Save, X, Trash2, LogOut, Plus, Edit, Loader2, Lock } from 'lucide-react';
...
const DieselStationMaster = () => {
    ...
    const handleAddNew = () => { if (!canAdd) return; setMode('add'); setFormData({ id: null, code: '', name: '', location: '', contact_no: '' }); };
    ...
    return (
        <div className={`space-y-6 text-text-main transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between">
                <div><h2 className="text-3xl font-bold gradient-text tracking-tight">Diesel Station Master</h2><p className="text-slate-400 mt-1">Manage diesel stations</p></div>
            ...
            <div className="glass-card max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="px-6 py-4 border-b border-white/5 bg-gradient-to-r from-yellow-600/20 to-orange-600/20">
                    <div className="flex items-center space-x-3"><Fuel className="w-6 h-6 text-yellow-400" /><h3 className="text-xl font-bold text-white italic">Diesel Station Master</h3></div>
                </div>
                ...
                <div className="p-6 space-y-5">
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <div className="col-span-3 flex space-x-2">
                            <input type="text" value={formData.code || ''} onChange={(e) => handleInputChange('code', e.target.value)} disabled={isViewMode} placeholder="e.g., DS01" className="w-28 input-modern disabled:opacity-70" />
                    ...
                    <div className="grid grid-cols-4 items-center gap-4"><label className="text-right text-slate-300 font-medium">Station Name :</label><div className="col-span-3"><input type="text" value={formData.name || ''} onChange={(e) => handleInputChange('name', e.target.value)} disabled={isViewMode} placeholder="Enter station name" className="w-full input-modern disabled:opacity-70" /></div></div>
                    <div className="grid grid-cols-4 items-center gap-4"><label className="text-right text-slate-300 font-medium">Location :</label><div className="col-span-3"><input type="text" value={formData.location || ''} onChange={(e) => handleInputChange('location', e.target.value)} disabled={isViewMode} placeholder="Enter location" className="w-full input-modern disabled:opacity-70" /></div></div>
                    <div className="grid grid-cols-4 items-center gap-4"><label className="text-right text-slate-300 font-medium">Contact No :</label><div className="col-span-3"><input type="text" value={formData.contact_no || ''} onChange={(e) => handleInputChange('contact_no', e.target.value)} disabled={isViewMode} placeholder="Enter contact number" className="w-48 input-modern disabled:opacity-70" /></div></div>
                </div>
