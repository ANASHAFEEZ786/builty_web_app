...
import { Layers, Search, ChevronLeft, ChevronRight, Save, X, Trash2, LogOut, Plus, Edit, Loader2, Lock } from 'lucide-react';
import { useDataService } from '../../lib/dataService';
import { usePermissions } from '../../lib/usePermissions';

const AccountsCategoryMaster = () => {
    const [mode, setMode] = useState('view');
    const [currentIndex, setCurrentIndex] = useState(0);
    const [isVisible, setIsVisible] = useState(false);
    const [formData, setFormData] = useState({ id: null, code: '', name: '', fs_category: '' });
    const [isSaving, setIsSaving] = useState(false);

    const { canAdd, canEdit, canDelete, canView } = usePermissions('accounts_categories');
    const { data: records, loading, error, create, update, remove, refresh } = useDataService('accounts_categories');
    ...
    const handleAddNew = () => { if (!canAdd) return; setMode('add'); setFormData({ id: null, code: '', name: '', fs_category: '' }); };
    const handleEdit = () => { if (!canEdit) return; setMode('edit'); };

    const handleSave = async () => {
        setIsSaving(true);
        try {
            const saveData = { code: formData.code, name: formData.name, fs_category: formData.fs_category };
    ...
    return (
        <div className={`space-y-6 text-text-main transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between">
                <div><h2 className="text-3xl font-bold gradient-text tracking-tight">Accounts Category</h2><p className="text-slate-400 mt-1">Manage account categories</p></div>
            ...
            <div className="glass-card max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="px-6 py-4 border-b border-white/5 bg-gradient-to-r from-violet-600/20 to-purple-600/20">
                    <div className="flex items-center space-x-3"><Layers className="w-6 h-6 text-violet-400" /><h3 className="text-xl font-bold text-white italic">Accounts Category</h3></div>
                </div>

                <div className="p-6 space-y-5">
                    <div className="grid grid-cols-4 items-center gap-4">
                        <label className="text-right text-slate-300 font-medium">Category Code :</label>
                        <div className="col-span-3 flex space-x-2">
                            <input type="text" value={formData.code || ''} onChange={(e) => handleInputChange('code', e.target.value)} disabled={isViewMode} placeholder="e.g., AC01" className="w-28 input-modern disabled:opacity-70" />
                    ...
                    <div className="grid grid-cols-4 items-center gap-4"><label className="text-right text-slate-300 font-medium">Category Name :</label><div className="col-span-3"><input type="text" value={formData.name || ''} onChange={(e) => handleInputChange('name', e.target.value)} disabled={isViewMode} placeholder="Enter category name" className="w-full input-modern disabled:opacity-70" /></div></div>
                    <div className="grid grid-cols-4 items-center gap-4"><label className="text-right text-slate-300 font-medium">FS Category :</label><div className="col-span-3"><select value={formData.fs_category || ''} onChange={(e) => handleInputChange('fs_category', e.target.value)} disabled={isViewMode} className="w-48 input-modern disabled:opacity-70"><option value="">Select...</option><option value="Balance Sheet">Balance Sheet</option><option value="Profit & Loss">Profit & Loss</option></select></div></div>
                </div>
...
export default AccountsCategoryMaster;
