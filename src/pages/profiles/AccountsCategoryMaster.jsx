...
import { Layers, Search, ChevronLeft, ChevronRight, Save, X, Trash2, LogOut, Plus, Edit } from 'lucide-react';
...
const AccountsCategoryMaster = () => {
    ...
    const [records, setRecords] = useState([
        { id: 1, code: '01', name: 'Current Assets', fsCategory: 'Balance Sheet' },
        { id: 2, code: '02', name: 'Fixed Assets', fsCategory: 'Balance Sheet' },
        { id: 3, code: '03', name: 'Liabilities', fsCategory: 'Balance Sheet' },
        { id: 4, code: '07', name: 'Revenue', fsCategory: 'Profit & Loss' },
        { id: 5, code: '08', name: 'Operating Expense', fsCategory: 'Profit & Loss' },
    ]);

    const [formData, setFormData] = useState(records[0]);
    const handleInputChange = (field, value) => setFormData({ ...formData, [field]: value });
    const handleAddNew = () => { setMode('add'); setFormData({ id: records.length + 1, code: '', name: '', fsCategory: '' }); };
    ...
    return (
        <div className={`space-y-6 text-text-main transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between"><div><h2 className="text-3xl font-bold gradient-text tracking-tight">Accounts Category Master</h2><p className="text-slate-400 mt-1">Manage account categories</p></div><span className="px-3 py-1.5 bg-slate-800/50 rounded-lg text-xs text-slate-400">Record {currentIndex + 1} of {records.length}</span></div>
            <div className="glass-card max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="px-6 py-4 border-b border-white/5 bg-gradient-to-r from-purple-600/20 to-violet-600/20"><div className="flex items-center space-x-3"><Layers className="w-6 h-6 text-purple-400" /><h3 className="text-xl font-bold text-white italic">Accounts Category Master</h3></div></div>
                <div className="p-6 space-y-5">
                    <div className="grid grid-cols-4 items-center gap-4"><label className="text-right text-slate-300 font-medium">Category Code :</label><div className="col-span-3 flex space-x-2"><input type="text" value={formData.code} onChange={(e) => handleInputChange('code', e.target.value)} disabled={isViewMode} className="w-24 input-modern disabled:opacity-70" /><button className="p-3 bg-slate-700/50 hover:bg-slate-700 rounded-xl transition-colors border border-slate-600/50"><Search className="w-5 h-5 text-slate-400" /></button></div></div>
                    ...
                    <div className="grid grid-cols-4 items-center gap-4"><label className="text-right text-slate-300 font-medium">FS Category :</label><div className="col-span-3"><select value={formData.fsCategory} onChange={(e) => handleInputChange('fsCategory', e.target.value)} disabled={isViewMode} className="w-full input-modern disabled:opacity-70"><option value="">Select</option><option value="Balance Sheet">Balance Sheet</option><option value="Profit & Loss">Profit & Loss</option></select></div></div>
                </div>
