...
import { Layers, Plus, Loader2, Save, X, AlertTriangle } from 'lucide-react';
...
const AccountsCategoryMaster = () => {
    ...
    const { canAdd, canEdit, canDelete, canView } = usePermissions('accounts_category');
    const { data: records, loading, error, create, update, remove, refresh } = useDataService('accounts_category');
    ...
    const handleAddNew = () => {
        if (!canAdd) return;
        setModalMode('add');
        setFormData({ id: null, code: getNextCode(), name: '' });
        setModalOpen(true);
    };
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            const saveData = { code: formData.code, name: formData.name };
            if (modalMode === 'add') { await create(saveData); toast.success('Category created!'); }
            else { await update(formData.id, saveData); toast.success('Category updated!'); }
            setModalOpen(false);
    ...
    const confirmDelete = async () => {
        ...
        try { await remove(deleteConfirm.id); toast.success('Category deleted!'); }
        catch (err) { toast.error('Error: ' + err.message); }
    ...
    return (
        <div className={`space-y-6 transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between">
                <div><h2 className="text-3xl font-bold gradient-text">Account Categories</h2><p className="text-slate-400 mt-1">Manage account categories</p></div>
            ...
            <div className="glass-card p-6">
                <div className="flex items-center gap-3 mb-6"><Layers className="w-6 h-6 text-indigo-400" /><h3 className="text-xl font-bold text-white">Categories</h3><span className="px-3 py-1 bg-slate-800/50 rounded-full text-sm text-slate-400">{records.length} records</span></div>
