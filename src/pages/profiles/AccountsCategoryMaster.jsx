...
import { Layers, Search, ChevronLeft, ChevronRight, Save, X, Trash2, LogOut, Plus, Edit, Loader2 } from 'lucide-react';
...
const AccountsCategoryMaster = () => {
    ...
    const handleAddNew = () => { setMode('add'); setFormData({ id: null, code: '', name: '', fs_category: '' }); };
    ...
    if (loading) return <div className="flex items-center justify-center h-64"><Loader2 className="w-8 h-8 animate-spin text-purple-500" /><span className="ml-3 text-slate-400">Loading...</span></div>;
    ...
    return (
        <div className={`space-y-6 text-text-main transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            <div className="glass-card max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="px-6 py-4 border-b border-white/5 bg-gradient-to-r from-purple-600/20 to-violet-600/20">
                    <div className="flex items-center space-x-3"><Layers className="w-6 h-6 text-purple-400" /><h3 className="text-xl font-bold text-white italic">Accounts Category Master</h3></div>
                </div>
                <div className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        <label className="text-right text-slate-300 font-medium">Category Name :</label>
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <div className="col-span-3">
                            <select value={formData.fs_category || ''} onChange={(e) => handleInputChange('fs_category', e.target.value)} disabled={isViewMode} className="w-full input-modern disabled:opacity-70">
