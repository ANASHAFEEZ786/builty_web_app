...
import { Package, Search, ChevronLeft, ChevronRight, Save, X, Trash2, LogOut, Plus, Edit, Loader2, Lock } from 'lucide-react';
...
const ItemMaster = () => {
    ...
    const [formData, setFormData] = useState({ id: null, code: '', name: '', unit: '', rate: 0, opening_stock: 0 });
    const [isSaving, setIsSaving] = useState(false);

    const { canAdd, canEdit, canDelete, canView } = usePermissions('items');
    const { data: records, loading, error, create, update, remove, refresh } = useDataService('items');
    ...
    const handleAddNew = () => { if (!canAdd) return; setMode('add'); setFormData({ id: null, code: '', name: '', unit: '', rate: 0, opening_stock: 0 }); };
    const handleEdit = () => { if (!canEdit) return; setMode('edit'); };

    const handleSave = async () => {
        setIsSaving(true);
        try {
            const saveData = { code: formData.code, name: formData.name, unit: formData.unit, rate: parseFloat(formData.rate) || 0, opening_stock: parseFloat(formData.opening_stock) || 0 };
    ...
    return (
        <div className={`space-y-6 text-text-main transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between">
                <div><h2 className="text-3xl font-bold gradient-text tracking-tight">Item Master</h2><p className="text-slate-400 mt-1">Manage stockable items</p></div>
            ...
            <div className="glass-card max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="px-6 py-4 border-b border-white/5 bg-gradient-to-r from-cyan-600/20 to-blue-600/20">
                    <div className="flex items-center space-x-3"><Package className="w-6 h-6 text-cyan-400" /><h3 className="text-xl font-bold text-white italic">Item Master</h3></div>
                </div>

                <div className="p-6 space-y-5">
                    <div className="grid grid-cols-4 items-center gap-4">
                        <label className="text-right text-slate-300 font-medium">Item Code :</label>
                        <div className="col-span-3 flex space-x-2">
                            <input type="text" value={formData.code || ''} onChange={(e) => handleInputChange('code', e.target.value)} disabled={isViewMode} placeholder="e.g., ITM001" className="w-28 input-modern disabled:opacity-70" />
                    ...
                    <div className="grid grid-cols-4 items-center gap-4"><label className="text-right text-slate-300 font-medium">Item Name :</label><div className="col-span-3"><input type="text" value={formData.name || ''} onChange={(e) => handleInputChange('name', e.target.value)} disabled={isViewMode} placeholder="Enter item name" className="w-full input-modern disabled:opacity-70" /></div></div>
                    <div className="grid grid-cols-4 items-center gap-4"><label className="text-right text-slate-300 font-medium">Unit :</label><div className="col-span-3"><input type="text" value={formData.unit || ''} onChange={(e) => handleInputChange('unit', e.target.value)} disabled={isViewMode} placeholder="e.g., Pcs, Kg, Ltr" className="w-32 input-modern disabled:opacity-70" /></div></div>
                    <div className="grid grid-cols-4 items-center gap-4"><label className="text-right text-slate-300 font-medium">Rate :</label><div className="col-span-3"><input type="number" value={formData.rate || 0} onChange={(e) => handleInputChange('rate', e.target.value)} disabled={isViewMode} placeholder="0.00" className="w-32 input-modern disabled:opacity-70" /></div></div>
                    <div className="grid grid-cols-4 items-center gap-4"><label className="text-right text-slate-300 font-medium">Opening Stock :</label><div className="col-span-3"><input type="number" value={formData.opening_stock || 0} onChange={(e) => handleInputChange('opening_stock', e.target.value)} disabled={isViewMode} placeholder="0" className="w-32 input-modern disabled:opacity-70" /></div></div>
                </div>
