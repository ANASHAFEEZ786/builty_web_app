...
import { Package, Plus, Loader2, Save, X, AlertTriangle } from 'lucide-react';
...
const ItemMaster = () => {
    ...
    const [formData, setFormData] = useState({ id: null, code: '', name: '', unit: '', rate: 0 });
    const [isSaving, setIsSaving] = useState(false);
    const [validationError, setValidationError] = useState('');
    const [deleteConfirm, setDeleteConfirm] = useState(null);

    const { canAdd, canEdit, canDelete, canView } = usePermissions('items');
    const { data: records, loading, error, create, update, remove, refresh } = useDataService('item');
    ...
    const handleAddNew = () => {
        if (!canAdd) return;
        setModalMode('add');
        setFormData({ id: null, code: getNextCode(), name: '', unit: '', rate: 0 });
        setModalOpen(true);
    };
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            const saveData = { code: formData.code, name: formData.name, unit: formData.unit, rate: parseFloat(formData.rate) || 0 };
            if (modalMode === 'add') { await create(saveData); toast.success('Item created!'); }
            else { await update(formData.id, saveData); toast.success('Item updated!'); }
            setModalOpen(false);
    ...
    const confirmDelete = async () => {
        ...
        try { await remove(deleteConfirm.id); toast.success('Item deleted!'); }
        catch (err) { toast.error('Error: ' + err.message); }
    ...
    const columns = [
        ...
        { key: 'name', label: 'Name' },
        { key: 'unit', label: 'Unit' },
        { key: 'rate', label: 'Rate', render: (val) => val ? `PKR ${Number(val).toLocaleString()}` : '-' }
    ];
    ...
    return (
        <div className={`space-y-6 transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between">
                <div><h2 className="text-3xl font-bold gradient-text">Item Master</h2><p className="text-slate-400 mt-1">Manage items and products</p></div>
            ...
            <div className="glass-card p-6">
                <div className="flex items-center gap-3 mb-6"><Package className="w-6 h-6 text-cyan-400" /><h3 className="text-xl font-bold text-white">Item List</h3><span className="px-3 py-1 bg-slate-800/50 rounded-full text-sm text-slate-400">{records.length} records</span></div>
                <DataTable columns={columns} data={records} onEdit={handleEdit} onDelete={handleDelete} canEdit={canEdit} canDelete={canDelete} />
            </div>

            <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={modalMode === 'add' ? 'Add Item' : 'Edit Item'} size="md">
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="block text-sm text-slate-300 mb-1.5">Unit</label><input type="text" value={formData.unit || ''} onChange={(e) => handleInputChange('unit', e.target.value)} className="w-full input-modern" placeholder="Kg, Pcs, etc" /></div>
                        <div><label className="block text-sm text-slate-300 mb-1.5">Rate</label><input type="number" value={formData.rate || 0} onChange={(e) => handleInputChange('rate', e.target.value)} className="w-full input-modern" /></div>
                    </div>
