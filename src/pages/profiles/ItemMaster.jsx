...
import { Package, Search, ChevronLeft, ChevronRight, Save, X, Trash2, LogOut, Plus, Edit, Loader2 } from 'lucide-react';
...
const ItemMaster = () => {
    ...
    const handleAddNew = () => { setMode('add'); setFormData({ id: null, code: '', name: '', unit: '', rate: 0 }); };
    ...
    if (loading) return <div className="flex items-center justify-center h-64"><Loader2 className="w-8 h-8 animate-spin text-indigo-500" /><span className="ml-3 text-slate-400">Loading...</span></div>;
    ...
    return (
        <div className={`space-y-6 text-text-main transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            <div className="glass-card max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="px-6 py-4 border-b border-white/5 bg-gradient-to-r from-indigo-600/20 to-blue-600/20">
                    <div className="flex items-center space-x-3"><Package className="w-6 h-6 text-indigo-400" /><h3 className="text-xl font-bold text-white italic">Item Master</h3></div>
                </div>
                <div className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        <label className="text-right text-slate-300 font-medium">Item Name :</label>
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        <label className="text-right text-slate-300 font-medium">Unit :</label>
                        <div className="col-span-3"><input type="text" value={formData.unit || ''} onChange={(e) => handleInputChange('unit', e.target.value)} disabled={isViewMode} placeholder="e.g., Liters, Kg, Pcs" className="w-48 input-modern disabled:opacity-70" /></div>
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                        <label className="text-right text-slate-300 font-medium">Rate :</label>
                        <div className="col-span-3"><input type="number" value={formData.rate || ''} onChange={(e) => handleInputChange('rate', e.target.value)} disabled={isViewMode} className="w-32 input-modern disabled:opacity-70" /></div>
                    </div>
                </div>
