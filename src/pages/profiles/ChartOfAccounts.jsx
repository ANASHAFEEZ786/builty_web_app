...
import { BookOpen, Search, ChevronLeft, ChevronRight, Save, X, Trash2, LogOut, Plus, Edit, Loader2, Lock } from 'lucide-react';
...
const ChartOfAccounts = () => {
    ...
    const handleAddNew = () => { if (!canAdd) return; setMode('add'); setFormData({ id: null, code: '', description: '', fs_category: '', accounts_type: '', accounts_category: '', accounts_location: '', accounts_sub_category: '', opening_balance: 0 }); };
    const handleEdit = () => { if (!canEdit) return; setMode('edit'); };

    const handleSave = async () => {
        setIsSaving(true);
        try {
            const saveData = { code: formData.code, description: formData.description, fs_category: formData.fs_category, accounts_type: formData.accounts_type, accounts_category: formData.accounts_category, accounts_location: formData.accounts_location, accounts_sub_category: formData.accounts_sub_category, opening_balance: parseFloat(formData.opening_balance) || 0 };
    ...
    return (
        <div className={`space-y-6 text-text-main transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between">
                <div><h2 className="text-3xl font-bold gradient-text tracking-tight">Chart of Accounts</h2><p className="text-slate-400 mt-1">Main account ledger</p></div>
            ...
            <div className="glass-card max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="px-6 py-4 border-b border-white/5 bg-gradient-to-r from-blue-600/20 to-indigo-600/20">
                    <div className="flex items-center space-x-3"><BookOpen className="w-6 h-6 text-blue-400" /><h3 className="text-xl font-bold text-white italic">Chart of Accounts</h3></div>
                </div>

                <div className="p-6 space-y-5">
                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-4">
                            <div className="grid grid-cols-3 items-center gap-2"><label className="text-right text-slate-300 font-medium text-sm">Code :</label><div className="col-span-2 flex space-x-2"><input type="text" value={formData.code || ''} onChange={(e) => handleInputChange('code', e.target.value)} disabled={isViewMode} placeholder="e.g., 1001" className="w-24 input-modern disabled:opacity-70 text-sm" /><button className="p-2 bg-slate-700/50 hover:bg-slate-700 rounded-lg transition-colors border border-slate-600/50"><Search className="w-4 h-4 text-slate-400" /></button></div></div>
                            <div className="grid grid-cols-3 items-center gap-2"><label className="text-right text-slate-300 font-medium text-sm">Description :</label><div className="col-span-2"><input type="text" value={formData.description || ''} onChange={(e) => handleInputChange('description', e.target.value)} disabled={isViewMode} placeholder="Account description" className="w-full input-modern disabled:opacity-70 text-sm" /></div></div>
                            <div className="grid grid-cols-3 items-center gap-2"><label className="text-right text-slate-300 font-medium text-sm">FS Category :</label><div className="col-span-2"><select value={formData.fs_category || ''} onChange={(e) => handleInputChange('fs_category', e.target.value)} disabled={isViewMode} className="w-full input-modern disabled:opacity-70 text-sm"><option value="">Select...</option><option value="Balance Sheet">Balance Sheet</option><option value="Profit & Loss">Profit & Loss</option></select></div></div>
                            <div className="grid grid-cols-3 items-center gap-2"><label className="text-right text-slate-300 font-medium text-sm">Acc Type :</label><div className="col-span-2"><select value={formData.accounts_type || ''} onChange={(e) => handleInputChange('accounts_type', e.target.value)} disabled={isViewMode} className="w-full input-modern disabled:opacity-70 text-sm"><option value="">Select...</option><option value="Assets">Assets</option><option value="Liabilities">Liabilities</option><option value="Equity">Equity</option><option value="Income">Income</option><option value="Expense">Expense</option></select></div></div>
                        </div>
                        <div className="space-y-4">
                            <div className="grid grid-cols-3 items-center gap-2"><label className="text-right text-slate-300 font-medium text-sm">Category :</label><div className="col-span-2"><input type="text" value={formData.accounts_category || ''} onChange={(e) => handleInputChange('accounts_category', e.target.value)} disabled={isViewMode} placeholder="Category" className="w-full input-modern disabled:opacity-70 text-sm" /></div></div>
                            <div className="grid grid-cols-3 items-center gap-2"><label className="text-right text-slate-300 font-medium text-sm">Sub Cat :</label><div className="col-span-2"><input type="text" value={formData.accounts_sub_category || ''} onChange={(e) => handleInputChange('accounts_sub_category', e.target.value)} disabled={isViewMode} placeholder="Sub category" className="w-full input-modern disabled:opacity-70 text-sm" /></div></div>
                            <div className="grid grid-cols-3 items-center gap-2"><label className="text-right text-slate-300 font-medium text-sm">Location :</label><div className="col-span-2"><input type="text" value={formData.accounts_location || ''} onChange={(e) => handleInputChange('accounts_location', e.target.value)} disabled={isViewMode} placeholder="Location" className="w-full input-modern disabled:opacity-70 text-sm" /></div></div>
                            <div className="grid grid-cols-3 items-center gap-2"><label className="text-right text-slate-300 font-medium text-sm">Opening Bal :</label><div className="col-span-2"><input type="number" value={formData.opening_balance || 0} onChange={(e) => handleInputChange('opening_balance', e.target.value)} disabled={isViewMode} placeholder="0.00" className="w-32 input-modern disabled:opacity-70 text-sm" /></div></div>
                        </div>
                    </div>
                </div>
