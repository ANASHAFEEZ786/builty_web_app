...
import { Users, Search, ChevronLeft, ChevronRight, Save, X, Trash2, LogOut, Plus, Edit, Loader2 } from 'lucide-react';
...
const DriverMaster = () => {
    ...
    const [formData, setFormData] = useState({
        id: null, driver_id: '', name: '', type: 'D', joining_date: '', reference_name: '', others: '',
        nic_no: '', nic_expiry: '', license_no: '', license_expiry: '', is_active: true
    });
    ...
    const handleAddNew = () => { setMode('add'); setFormData({ id: null, driver_id: '', name: '', type: 'D', joining_date: '', reference_name: '', others: '', nic_no: '', nic_expiry: '', license_no: '', license_expiry: '', is_active: true }); };
    ...
    if (loading) return <div className="flex items-center justify-center h-64"><Loader2 className="w-8 h-8 animate-spin text-teal-500" /><span className="ml-3 text-slate-400">Loading drivers...</span></div>;
        ...
        <div className={`space-y-6 text-text-main transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            <div className="glass-card max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="px-6 py-4 border-b border-white/5 bg-gradient-to-r from-teal-600/20 to-cyan-600/20">
                    <div className="flex items-center space-x-3"><Users className="w-6 h-6 text-teal-400" /><h3 className="text-xl font-bold text-white italic">Driver / Cleaner Master</h3></div>
                </div>
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="flex items-center gap-4">
                            <label className="w-32 text-right text-slate-300 font-medium">ID :</label>
                            <div className="flex space-x-2 flex-1">
                                <input type="text" value={formData.driver_id || ''} onChange={(e) => handleInputChange('driver_id', e.target.value)} disabled={isViewMode} placeholder="D-0001" className="w-28 input-modern disabled:opacity-70" />
                        ...
                        <div className="flex items-center gap-4">
                            <label className="w-32 text-right text-slate-300 font-medium">Name :</label>
                            <input type="text" value={formData.name || ''} onChange={(e) => handleInputChange('name', e.target.value)} disabled={isViewMode} className="flex-1 input-modern disabled:opacity-70" />
                        </div>
                        <div className="flex items-center gap-4">
                            <label className="w-32 text-right text-slate-300 font-medium">Type :</label>
                            <select value={formData.type || 'D'} onChange={(e) => handleInputChange('type', e.target.value)} disabled={isViewMode} className="w-48 input-modern disabled:opacity-70">
                                <option value="D">Driver (D)</option>
                                <option value="C">Cleaner (C)</option>
                            </select>
                        </div>
                        <div className="flex items-center gap-4">
                            <label className="w-32 text-right text-slate-300 font-medium">Joining Date :</label>
                            <input type="date" value={formData.joining_date || ''} onChange={(e) => handleInputChange('joining_date', e.target.value)} disabled={isViewMode} className="w-48 input-modern disabled:opacity-70" />
                        </div>
                        <div className="flex items-center gap-4">
                            <label className="w-32 text-right text-slate-300 font-medium">Reference :</label>
                            <input type="text" value={formData.reference_name || ''} onChange={(e) => handleInputChange('reference_name', e.target.value)} disabled={isViewMode} className="flex-1 input-modern disabled:opacity-70" />
                        </div>
                        <div className="flex items-center gap-4">
                            <label className="w-32 text-right text-slate-300 font-medium">Active :</label>
                            <input type="checkbox" checked={formData.is_active} onChange={(e) => handleInputChange('is_active', e.target.checked)} disabled={isViewMode} className="w-5 h-5 rounded border-slate-600" />
                        </div>
                    </div>

                    {/* NIC Detail */}
                    <div className="border border-slate-700/50 rounded-xl p-4 space-y-4">
                        <h4 className="text-sm font-semibold text-teal-400 uppercase tracking-wider">NIC Detail</h4>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="flex items-center gap-4">
                                <label className="w-32 text-right text-slate-300 font-medium">NIC No :</label>
                                <input type="text" value={formData.nic_no || ''} onChange={(e) => handleInputChange('nic_no', e.target.value)} disabled={isViewMode} placeholder="35201-1234567-1" className="flex-1 input-modern disabled:opacity-70" />
                            </div>
                            <div className="flex items-center gap-4">
                                <label className="w-32 text-right text-slate-300 font-medium">NIC Expiry :</label>
                                <input type="date" value={formData.nic_expiry || ''} onChange={(e) => handleInputChange('nic_expiry', e.target.value)} disabled={isViewMode} className="w-48 input-modern disabled:opacity-70" />
                            </div>
                        </div>
                    </div>
                    ...
                    {/* License Detail */}
                    <div className="border border-slate-700/50 rounded-xl p-4 space-y-4">
                        <h4 className="text-sm font-semibold text-cyan-400 uppercase tracking-wider">License Detail</h4>
                        <div className="grid grid-cols-2 gap-4">
                            <div className="flex items-center gap-4">
                                <label className="w-32 text-right text-slate-300 font-medium">License No :</label>
                                <input type="text" value={formData.license_no || ''} onChange={(e) => handleInputChange('license_no', e.target.value)} disabled={isViewMode} className="flex-1 input-modern disabled:opacity-70" />
                            </div>
                            <div className="flex items-center gap-4">
                                <label className="w-32 text-right text-slate-300 font-medium">License Expiry :</label>
                                <input type="date" value={formData.license_expiry || ''} onChange={(e) => handleInputChange('license_expiry', e.target.value)} disabled={isViewMode} className="w-48 input-modern disabled:opacity-70" />
                            </div>
                        </div>
                    </div>
                </div>
