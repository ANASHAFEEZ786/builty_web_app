...
import { Receipt, Search, ChevronLeft, ChevronRight, Save, X, Trash2, LogOut, Plus, Edit } from 'lucide-react';
...
const ExpenseMaster = () => {
    ...
    const [records, setRecords] = useState([
        { id: 1, code: '01', name: 'FUEL EXPENSE' },
        { id: 2, code: '02', name: 'TOLL TAX' },
        { id: 3, code: '03', name: 'LOADING CHARGES' },
        { id: 4, code: '04', name: 'UNLOADING CHARGES' },
        { id: 5, code: '05', name: 'DRIVER SALARY' },
        { id: 6, code: '06', name: 'MAINTENANCE' },
        { id: 7, code: '07', name: 'PARKING' },
        { id: 8, code: '08', name: 'POLICE CHALLAN' },
        { id: 9, code: '09', name: 'NILL' },
    ]);

    const [formData, setFormData] = useState(records[0]);

    const handleInputChange = (field, value) => setFormData({ ...formData, [field]: value });
    const handleAddNew = () => { setMode('add'); setFormData({ id: records.length + 1, code: '', name: '' }); };
    ...
    return (
        <div className={`space-y-6 text-text-main transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            <div className="glass-card max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="px-6 py-4 border-b border-white/5 bg-gradient-to-r from-orange-600/20 to-amber-600/20">
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                        ...
                        <div className="col-span-3">
                            <input type="text" value={formData.name} onChange={(e) => handleInputChange('name', e.target.value)} disabled={isViewMode} className="w-full input-modern disabled:opacity-70" placeholder="Enter expense name" />
                        </div>
                    </div>
                </div>
