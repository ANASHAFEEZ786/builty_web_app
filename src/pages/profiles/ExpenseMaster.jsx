...
import { Receipt, Plus, Loader2, Save, X, AlertTriangle } from 'lucide-react';
...
const ExpenseMaster = () => {
    ...
    const [formData, setFormData] = useState({ id: null, code: '', description: '', category: '' });
    const [isSaving, setIsSaving] = useState(false);
    const [validationError, setValidationError] = useState('');
    const [deleteConfirm, setDeleteConfirm] = useState(null);

    const { canAdd, canEdit, canDelete, canView } = usePermissions('expense');
    const { data: records, loading, error, create, update, remove, refresh } = useDataService('expense');
    ...
    const handleAddNew = () => {
        if (!canAdd) return;
        setModalMode('add');
        setFormData({ id: null, code: getNextCode(), description: '', category: '' });
        setValidationError('');
        setModalOpen(true);
    };
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            const saveData = { code: formData.code, description: formData.description, category: formData.category };
            if (modalMode === 'add') { await create(saveData); toast.success('Expense created!'); }
            else { await update(formData.id, saveData); toast.success('Expense updated!'); }
            setModalOpen(false);
    ...
    const confirmDelete = async () => {
        ...
        try { await remove(deleteConfirm.id); toast.success('Expense deleted!'); }
        catch (err) { toast.error('Error: ' + err.message); }
    ...
    return (
        <div className={`space-y-6 transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <div className="flex items-center justify-between">
                <div><h2 className="text-3xl font-bold gradient-text">Expense Types</h2><p className="text-slate-400 mt-1">Manage expense categories</p></div>
            ...
            <div className="glass-card p-6">
                <div className="flex items-center gap-3 mb-6"><Receipt className="w-6 h-6 text-orange-400" /><h3 className="text-xl font-bold text-white">Expense List</h3><span className="px-3 py-1 bg-slate-800/50 rounded-full text-sm text-slate-400">{records.length} records</span></div>
                <DataTable columns={columns} data={records} onEdit={handleEdit} onDelete={handleDelete} canEdit={canEdit} canDelete={canDelete} />
            </div>

            <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={modalMode === 'add' ? 'Add Expense' : 'Edit Expense'} size="md">
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div><label className="block text-sm text-slate-300 mb-1.5">Category</label><input type="text" value={formData.category || ''} onChange={(e) => handleInputChange('category', e.target.value)} className="w-full input-modern" /></div>
                    </div>
